...
NativeMessageBridge::NativeMessageBridge(QLocalSocket *socket, NativeMessaging *nativeMessaging, QObject *parent)
    : QObject{parent}
    , m_socket{socket}
    , m_nativeMessaging{nativeMessaging}
    , m_reassembler{new NativeMessageReassembler(this)}
{
    m_socket->setParent(this);

    connect(m_socket, &QLocalSocket::readyRead, this, [this]() {
        m_reassembler->addData(m_socket->readAll());
    });
    ...
    connect(m_socket, &QLocalSocket::disconnected, this, &NativeMessageBridge::primaryDisconnected);
}
